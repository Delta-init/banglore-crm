...
/**
 * Authenticates requests from the CallRecorder Android app.
 * Caller must send:   x-api-key: <CALL_RECORDER_API_KEY from .env>
 */
export const authenticateDeviceKey = (
  req: Request,
  res: Response,
  next: NextFunction,
): void => {
  const apiKey     = req.headers["x-api-key"];
  const expectedKey = process.env.CALL_RECORDER_API_KEY;

  if (!expectedKey) {
    sendError(res, "Call recorder integration is not configured on this server", 503);
    return;
  }

  if (!apiKey || apiKey !== expectedKey) {
    sendError(res, "Invalid or missing device API key", 401);
    return;
  }
