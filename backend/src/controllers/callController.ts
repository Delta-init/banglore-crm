...
export const RECORDINGS_DIR = path.join(process.cwd(), "uploads", "recordings");
...
export const recordingUpload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    const ok =
      ...
      file.originalname.endsWith(".aac")  ||
...
export const uploadRecording = async (
  ...
  try {
    ...
    // ── Resolve agent name from extension ─────────────────────────────────
    let agentName: string | null = null;
    if (agent_extension) {
      const { User } = await import("../models/User.js");
      const user = await User.findOne({ extension: agent_extension })
        .select("name")
    ...
    if (agent_extension) {
      try {
        const { User } = await import("../models/User.js");
        const agentUser = await User.findOne({ extension: agent_extension })
          .select("_id")
        ...
        // Non-fatal — socket emit failure must never block the HTTP response
      }
    }

    sendSuccess(res, "Call log created", {
      call_log_id:   log._id.toString(),
